...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   //below is a bunch of classifications that ultimately get used to differentiate between a ball and a strike.
   ...
   //We've talked about ball versus strike
   ...
   Use["LD"]              = 1; // Linear Discriminant identical to Fisher.   performs some linear combinations with weighting--can understand from the weights it assigns, which variables it thinks are the most important versus not so useful
   Use["Fisher"]          = 0;
   Use["FisherG"]         = 0;
   ...
   TString fname = "./StockChanges-ND.csv.root";
   ...
   TTree *signalTree     = (TTree*)input->Get("ntuple"); //could be two different files, one with only signal, and one with only background. This is another way of doing
   //what we do later with mycuts, and mycutb. 
   TTree *background     = (TTree*)input->Get("ntuple");
   ...
   dataloader->AddVariable( "GM", "GM Daily Change", "%", 'F');
   dataloader->AddVariable( "X", "X Daily Change" , 'F' );
   dataloader->AddVariable( "TSLA", "TSLA Daily Change", 'F');
   dataloader->AddVariable( "TM", "TM Daily Change" , 'F' );
   dataloader->AddVariable( "RS", "RS Daily Change" , 'F' );
   dataloader->AddVariable( "NUE", "NUE Daily Change" , 'F' );
   dataloader->AddVariable( "HMC", "HMC Daily Change" , 'F' );
   dataloader->AddVariable( "F", "F Daily Change" , 'F' );




   dataloader->AddSpectator( "TMND", "Delta Tomorrow for X" , 'F' );


//you add the ND as a spectator, so that you can CUT on it, but you don't want the neural network to know about it. So you add it as a spectator.
  //But it doesn't give it to the neural net. These are things you want to be able to do stuff with, but not give to the classifiers as inputs




   //so the idea is if we give them these 9 coordinates it will be able to figure out if its a ball or a strike. Read chapter 8 of the users guide. 
...
//conditions in the market that cause the sotck to change positively alot. Thats the kinda event that we'd like to put in. Whereas the change is just a fractional percent
   //that's not as much of an interest to us.
...
//So the signal events are going to require the event has the S characterization for strike. And that the background events have "B". 
   ...
   //if you want to change the number of events for training and testing, use 1000 signal and 1000 bkg for training, if u want to adjust
   //this is where u can do that.
